...
function getAspectRatio(metadata) {
	const frame = metadata.other

	let [width, height] = (frame['fc:frame:image:aspect_ratio'] ?? frame['of:image:aspect_ratio'] ?? '1.91:1').split(':').map(Number)
...
export function Frame({ metadata, url, iframeRendererUrl = 'https://i.frames.fun/embed' }) {
		...
		<div className="flex mt-2">
			<div className="w-full max-w-[600px] mx-auto space-y-2">
				...
				{process.env.NODE_ENV === 'development' ? (
					<div className="flex w-full bg-slate-100 rounded items-center justify-center" style={{ aspectRatio }}>
						<span className="text-slate-500">On production, a frame will be shown here</span>
					</div>
				) : (
					<iframe
						className="w-full"
